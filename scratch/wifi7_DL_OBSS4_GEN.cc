...
Time simulationTime{"2s"}; //seconds
...
uint32_t nBSSs = 5;
int mcs_OBSS = 7;
double traffic_load_targetBSS{1.0};  // Factor determin the chanel congestion on the band of different BSSs
double traffic_load_BSS2{0.9};  // Factor determin the chanel congestion on the band of different BSSs
double traffic_load_BSS3{0.9};  // Factor determin the chanel congestion on the band of different BSSs
double traffic_load_BSS4{0.9};  // Factor determin the chanel congestion on the band of different BSSs
double traffic_load_BSS5{0.9};  // Factor determin the chanel congestion on the band of different BSSs
...
double powAp1 = 20;        // dBm
double powAp2 = 20;        // dBm
double powAp3 = 20;        // dBm
double powAp4 = 20;        // dBm
double powAp5 = 20;        // dBm
...
cmd.AddValue("traffic_load_targetBSS", "Target BSS congestion level", traffic_load_targetBSS);
cmd.AddValue("traffic_load_BSS2", "OBSS 1 congestion level", traffic_load_BSS2);
cmd.AddValue("traffic_load_BSS3", "OBSS 2 congestion level", traffic_load_BSS3);
cmd.AddValue("traffic_load_BSS4", "OBSS 3 congestion level", traffic_load_BSS4);
cmd.AddValue("traffic_load_BSS5", "OBSS 4 congestion level", traffic_load_BSS4);
...
for (std::size_t run = 0; run < Number_runs; ++run) 
{
    ...
    {  
        traffic_load_BSS2 = ObssLoads[load];  // Factor determine the chanel congestion on the band of different BSSs
        traffic_load_BSS3 = ObssLoads[load];
        traffic_load_BSS4 = ObssLoads[load];
        traffic_load_BSS5 = ObssLoads[load];
        ...
        wifiStaNodesE.Create(nStations);
        ...
        // BSS E
        std::array<std::string, 3> channelStrE;
        std::array<FrequencyRange, 3> freqRangesE;
        uint64_t nonHtRefRateMbpsE = EhtPhy::GetNonHtReferenceRate(mcs_OBSS) / 1e6;
        std::string dataModeStrE = "EhtMcs" + std::to_string(mcs_OBSS);
        std::string ctrlRateStrE;           
        uint8_t nLinksE = 0;
        Ssid ssidE = Ssid("ns3-80211be-E");
        ...
        {   
            ...
            channelStrE[nLinksE] = "{0, " + std::to_string(channelWidth) + ", ";
            if (freq == 6)
            {
                channelStrE[nLinksE] += "BAND_6GHZ, 0}";
                freqRangesE[nLinksE] = WIFI_SPECTRUM_6_GHZ;
                Config::SetDefault("ns3::FriisPropagationLossModel::Frequency",
                                        DoubleValue(6e9));
                wifi.SetRemoteStationManager(nLinksE,
                                            "ns3::ConstantRateWifiManager",
                                            "DataMode", StringValue(dataModeStrE),
                                            "ControlMode", StringValue(dataModeStrE));
            }
            else if (freq == 5)
            {
                channelStrE[nLinksE] += "BAND_5GHZ, 0}";
                freqRangesE[nLinksE] = WIFI_SPECTRUM_5_GHZ;
                ctrlRateStrE = "OfdmRate" + std::to_string(nonHtRefRateMbpsE) + "Mbps";
                Config::SetDefault("ns3::FriisPropagationLossModel::Frequency",
                                    DoubleValue(5.180e9));
                wifi.SetRemoteStationManager(nLinksE,
                                            "ns3::ConstantRateWifiManager",
                                            "DataMode", StringValue(dataModeStrE),
                                            "ControlMode", StringValue(ctrlRateStrE));
            }
            else if (freq == 2.4)
            {
                channelStrE[nLinksE] += "BAND_2_4GHZ, 0}";
                freqRangesE[nLinksE] = WIFI_SPECTRUM_2_4_GHZ;
                Config::SetDefault("ns3::FriisPropagationLossModel::Frequency",
                                        DoubleValue(2.4e9));
                ctrlRateStrE = "ErpOfdmRate" + std::to_string(nonHtRefRateMbpsE) + "Mbps";
                wifi.SetRemoteStationManager(nLinksE,
                                            "ns3::ConstantRateWifiManager",
                                            "DataMode", StringValue(dataModeStrE),
                                            "ControlMode", StringValue(ctrlRateStrE));
        ...
        SpectrumWifiPhyHelper spectrumWifiPhyE(nLinksE);
        spectrumWifiPhyE.SetPcapDataLinkType(WifiPhyHelper::DLT_IEEE802_11_RADIO);
        spectrumWifiPhyE.Set("ChannelSwitchDelay", TimeValue(MicroSeconds(channelSwitchDelayUsec)));

        for (uint8_t linkId = 0; linkId < nLinksE; linkId++)
        {
            spectrumWifiPhyE.Set(linkId, "ChannelSettings", StringValue(channelStrE[linkId]));
            spectrumWifiPhyE.SetErrorRateModel ("ns3::TableBasedErrorRateModel");
            spectrumWifiPhyE.AddChannel(ChannelSet[linkId], freqRangesE[linkId]);
        }
        ...
        mac.SetType("ns3::StaWifiMac", "Ssid", SsidValue(ssidE));
        mac.SetEmlsrManager("ns3::DefaultEmlsrManager",
                        "EmlsrLinkSet",
                            //StringValue(emlsrLinks),
                            AttributeContainerValue<UintegerValue>(emlsrLinks),
                            ...
                            "AuxPhyChannelWidth",
                            UintegerValue(auxPhyChWidth));
        NetDeviceContainer staDevicesE = wifi.Install(spectrumWifiPhyE, mac, wifiStaNodesE);
        ...
        NetDeviceContainer apDeviceE = wifi.Install(spectrumWifiPhyE, mac, WifiApNodes.Get(4));

        Ptr<WifiNetDevice> ap5Device = apDeviceE.Get(0)->GetObject<WifiNetDevice>();
        if (enableObssPd)
        {
            ap5Device->GetHeConfiguration()->SetAttribute("BssColor", UintegerValue(5));
        }
        ...
        positionAlloc->Add(Vector(0.0, -dAP, 0.0)); // OBSS D
        ...
        */
        ...
        positionAlloc->Add(Vector(dAP, -dAP, 0.0)); // OBSS D
        ...
        positionAlloc->Add(Vector(dSta, -dAP+dSta, 0.0)); // STA4
        positionAlloc->Add(Vector(dAP+dSta,-dAP , 0.0)); // STA5
        ...
        staNodeInterfacesE = address.Assign(staDevicesE);
        ...
        double  maxRate_lower_BSS1 = maxRate * traffic_load_targetBSS;
        ...
        // BSS2
        {
        const auto maxRate_OBSS =  EhtPhy::GetDataRate(mcs_OBSS, channelWidth, gi, 1) / nStations;
        ...
        double  maxRate_lower_BSS2 = maxRate_OBSS * traffic_load_BSS2;
        ...
        // BSS3
        {
        const auto maxRate_OBSS3 =  EhtPhy::GetDataRate(mcs_OBSS, channelWidth, gi, 1) / nStations;
        ...
        double  maxRate_lower_BSS3 = maxRate_OBSS3 * traffic_load_BSS3;
        ...
        // BSS4
        {
        const auto maxRate_OBSS4 =  EhtPhy::GetDataRate(mcs_OBSS, channelWidth, gi, 1) / nStations;
        ...
        double  maxRate_lower_BSS4 = maxRate_OBSS4 * traffic_load_BSS4;
        ...
        // BSS5
        {
        const auto maxRate_OBSS5 =  EhtPhy::GetDataRate(mcs_OBSS, channelWidth, gi, 1) / nStations;
        ...
        double  maxRate_lower_BSS5 = maxRate_OBSS5 * traffic_load_BSS5;
    ...
    for (size_t j = 0; j < loads_size; ++j) 
    {
        std::cout << Throughput_analysis[j][run] << "\t";
    }
    std::cout << std::endl;
}
