...
uint32_t targetPacketCount = 30000;
...
cmd.AddValue("antennas", "Number of antennas for spatial multiplexing",  antennas);
...
double ObssLoads[] = {0.1}; // Array with specific values
...
for (std::size_t run = 0; run < Number_runs; ++run) 
{
    ...
    {  
        ...
        std::cout << "traffic_load_BBS " << traffic_load_BBS2 << std::endl;
        ...
        for (uint8_t linkId = 0; linkId < nLinksA; linkId++)
        {
            spectrumWifiPhyA.Set(linkId, "Antennas", UintegerValue(antennas)); // At least 2 antennas for 2 RF chains
            spectrumWifiPhyA.Set(linkId, "MaxSupportedRxSpatialStreams", UintegerValue(antennas)); // Support 2 Rx streams
            spectrumWifiPhyA.Set(linkId, "MaxSupportedTxSpatialStreams", UintegerValue(antennas)); // Support 2 Tx streams
        }
        ...
        spectrumWifiPhyB.Set("Antennas", UintegerValue(antennas)); // At least 2 antennas for 2 RF chains
        spectrumWifiPhyB.Set("MaxSupportedRxSpatialStreams", UintegerValue(antennas)); // Support 2 Rx streams
        spectrumWifiPhyB.Set("MaxSupportedTxSpatialStreams", UintegerValue(antennas)); // Support 2 Tx streams
        ...
        if (nLinksB == 2 || nLinksB == 3)
        {
            ...
            spectrumWifiPhyD.Set("Antennas", UintegerValue(antennas)); // At least 2 antennas for 2 RF chains
            spectrumWifiPhyD.Set("MaxSupportedRxSpatialStreams", UintegerValue(antennas)); // Support 2 Rx streams
            spectrumWifiPhyD.Set("MaxSupportedTxSpatialStreams", UintegerValue(antennas)); // Support 2 Tx streams
            ...
            apDeviceD = wifiD.Install(spectrumWifiPhyD, macD, WifiApNodes.Get(2));
        ...
        {   
            ...
            spectrumWifiPhyF.Set("Antennas", UintegerValue(antennas)); // At least 2 antennas for 2 RF chains
            spectrumWifiPhyF.Set("MaxSupportedRxSpatialStreams", UintegerValue(antennas)); // Support 2 Rx streams
            spectrumWifiPhyF.Set("MaxSupportedTxSpatialStreams", UintegerValue(antennas)); // Support 2 Tx streams
            ...
            apDeviceF = wifiF.Install(spectrumWifiPhyF, macF, WifiApNodes.Get(3));
        ...
        if (nLinksB == 2 || nLinksB == 3)
        {
            std::cout << "Da5al el 2 for mobility" << std::endl;
            positionAlloc->Add(Vector(0.0, dAP, 0.0));  // AP3
        }
        if (nLinksB == 3)
        {
            std::cout << "Da5al el 3 for mobility" << std::endl;
            positionAlloc->Add(Vector(dAP, dAP, 0.0));  // AP5

        }

        positionAlloc->Add(Vector(dSta, dSta, 0.0));  // TSTA
        //positionAlloc->Add(Vector(2*dSta, 2*dSta, 0.0));  // TSTA2

        positionAlloc->Add(Vector(dSta+dAP, dSta, 0.0));    // STA1
        positionAlloc->Add(Vector((2*dSta)+dAP, (2*dSta), 0.0));    // STA2
        if (nLinksB == 2 || nLinksB == 3)
        {
            std::cout << "Msh hayd5ol hena" << std::endl;
            positionAlloc->Add(Vector(dSta, dSta+dAP, 0.0));  // StA3
            positionAlloc->Add(Vector((2*dSta), (2*dSta)+dAP, 0.0));  // StA3
        }
        if (nLinksB == 3)
        {
