...
std::ofstream throughputOBSSFile;
std::ofstream latencyOBSSFile;
...
std::vector<std::vector<double>> Throughput_analysis(nBSSs, std::vector<double>(Number_runs));
std::vector<std::vector<double>> Latency_analysis(nBSSs, std::vector<double>(Number_runs));

throughputOBSSFile.open("throughput_mtx.txt");
latencyOBSSFile.open("latency_mtx.txt");
...
for (std::size_t run = 0; run < Number_runs; ++run) 
{
    ...
    RngSeedManager::SetSeed(seedVector[run]); // Set a specific seed
    ...
    {   
        ...
        Throughput_analysis[counter][run] = through;
        Latency_analysis[counter][run] = delay;
        counter++;
    }
    Simulator::Destroy();
}
...
throughputOBSSFile.flush();
latencyOBSSFile.flush();


// Write the matrix in a space-separated format
for (size_t i = 0; i < nBSSs; ++i) {
    for (size_t j = 0; j < Number_runs; ++j) {
        throughputOBSSFile << Throughput_analysis[i][j] << " ";
    }
    throughputOBSSFile << std::endl;  // New line after each row
}
for (size_t i = 0; i < nBSSs; ++i) {
    for (size_t j = 0; j < Number_runs; ++j) {
        latencyOBSSFile << Latency_analysis[i][j] << " ";
    }
    latencyOBSSFile << std::endl;  // New line after each row
}

throughputOBSSFile.close();
latencyOBSSFile.close();
