...
for (std::size_t run = 0; run < Number_runs; ++run) 
{
    ...
    {  
        std::cout << ObssLoads[load] << std::endl;
        ...
        positionAlloc->Add(Vector(-dAP, 0.0, 0.0));  // AP3

        positionAlloc->Add(Vector(dSta, dSta, 0.0));  // STA1
        positionAlloc->Add(Vector(dSta+dAP, dSta, 0.0));   // STA2
        positionAlloc->Add(Vector(-dAP+dSta,dSta , 0.0));    // STA3
