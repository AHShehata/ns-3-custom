...
main(int argc, char* argv[])
{
    ...
    cmd.AddValue("verbose", "Allow ", verbose);
    cmd.Parse(argc, argv);
    ...
    for (uint16_t i = startIndex; i <= stopIndex; i++)
    {
        ...
        else if (i == 9)
        {
            ...
            int channelWidth = 20;
            int gi = 400;
            const auto maxLoad = HtPhy::GetDataRate(mcs, channelWidth, gi, 1)/ 1000000;
        ...
        if (wifiType == "ns3::YansWifiPhy")
        {
            ...
            //phy.Set("ChannelSettings",
            //        StringValue(std::string("{0, ") + (i <= 15 ? "20" : "40") + ", BAND_2_4GHZ, 0}"));
            //phy.Set("ChannelSettings",
            //        StringValue(std::string("{0, ") + (i <= 15 ? "20" : "40") + ", BAND_6GHZ, 0}"));
        ...
        std::cout << "Number of received packets " << g_samples << std::endl;
        Simulator::Destroy();
    }
    return 0;
}
