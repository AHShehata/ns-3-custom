...
spectrumPhy.SetErrorRateModel ("ns3::LogSgnErrorRateModel", "ChannelModelType",StringValue("ChannelModelB"));

spectrumPhy.Set("Antennas", UintegerValue(2)); // At least 2 antennas for 2 RF chains
spectrumPhy.Set("MaxSupportedRxSpatialStreams", UintegerValue(2)); // Support 2 Rx streams
spectrumPhy.Set("MaxSupportedTxSpatialStreams", UintegerValue(2)); // Support 2 Tx streams
...
spectrumPhy.Set("Antennas", UintegerValue(1)); // At least 2 antennas for 2 RF chains
spectrumPhy.Set("MaxSupportedRxSpatialStreams", UintegerValue(1)); // Support 2 Rx streams
spectrumPhy.Set("MaxSupportedTxSpatialStreams", UintegerValue(1)); // Support 2 Tx streams
...
AP_phy->GetAttribute("MaxSupportedRxSpatialStreams", Rxstreams);
...
distStr << "ns3::UniformRandomVariable[Min=0.0|Max=" << distance << "]";
mobility.SetPositionAllocator("ns3::RandomDiscPositionAllocator",
    "X", DoubleValue(0.0), // AP1 at center
    "Y", DoubleValue(0.0), // AP1 at center
    "Rho",StringValue(distStr.str()));
mobility.SetMobilityModel("ns3::ConstantPositionMobilityModel");
...
for (std::size_t i = 0; i < nStations; i++)
{
    ...
    streamNumber += server.AssignStreams(wifiStaNodes.Get(i), streamNumber);      
}
*/
...
std::cout << "Inter arrival time between packets " << packetInterval << std::endl;
//4294967295U
...
for (uint32_t i = 0; i < serverApp.GetN(); i++)
{
    ...
    std::cout << "Received packets by sta " << i+1 << " " << DynamicCast<UdpServer>(serverApp.Get(i))->GetReceived() << std::endl;
