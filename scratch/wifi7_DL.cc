...
for (uint8_t linkId = 0; linkId < nLinks; linkId++)
{
    spectrumWifiPhy.Set(linkId, "Antennas", UintegerValue(2)); // At least 2 antennas for 2 RF chains
    spectrumWifiPhy.Set(linkId, "MaxSupportedRxSpatialStreams", UintegerValue(2)); // Support 2 Rx streams
    spectrumWifiPhy.Set(linkId, "MaxSupportedTxSpatialStreams", UintegerValue(2)); // Support 2 Tx streams
}
...
/* Applications */
auto maxRate =   EhtPhy::GetDataRate(mcs, channelWidth, gi, 1);
 //        maxRate = 50 * 1000000;
...
std::cout << "Number of txed frames on link 1 " <<  txCount_link1 << std::endl;
std::cout << "Number of txed frames on link 2 " <<  txCount_link2 << std::endl;
