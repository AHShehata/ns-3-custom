...
cmd.AddValue("nBSSs", "Nulber of BSSs in the network", nBSSs);
...
for (std::size_t run = 0; run < Number_runs; ++run) 
{
    ...
    {  
        ...
        {
            std::cout << "Da5al el 2 for devicees" << std::endl;
            // BSS D
            WifiHelper wifiD;
            WifiMacHelper macD;
        ...
        if (nLinksB == 3)
        {   
            std::cout << "Da5al el 3 for devicees" << std::endl;
            // BSS F
            WifiHelper wifiF;
            WifiMacHelper macF;
...
positionAlloc->Add(Vector(0.0, 0.0, 0.0));  // AP1 at (0, 0)
...
mobility.SetPositionAllocator("ns3::RandomDiscPositionAllocator",
                                "X", DoubleValue(0.0), // AP1 at center
                                "Y", DoubleValue(0.0), // AP1 at center
                                "Rho",StringValue("ns3::UniformRandomVariable[Min=0.0|Max=10.0]"));
mobility.SetMobilityModel("ns3::ConstantPositionMobilityModel");
mobility.Install(wifiStaNodesA);

Ptr<UniformRandomVariable> radius2 = CreateObject<UniformRandomVariable>();
radius2->SetAttribute("Min", DoubleValue(0.0));
radius2->SetAttribute("Max", DoubleValue(dSta)); // 10-meter radius for AP2
mobility.SetPositionAllocator("ns3::RandomDiscPositionAllocator",
                                "X", DoubleValue(dAP), // AP2 at center
                                "Y", DoubleValue(0.0),  // AP2 at center
                                "Rho",StringValue("ns3::UniformRandomVariable[Min=0.0|Max=10.0]"));
mobility.SetMobilityModel("ns3::ConstantPositionMobilityModel");
mobility.Install(wifiStaNodesB);
if (nLinksB == 2 || nLinksB == 3)
{
    // Configure mobility for STAs of AP2 (randomly distributed around AP2 within 10 meters)
    Ptr<UniformRandomVariable> radius3 = CreateObject<UniformRandomVariable>();
    radius3->SetAttribute("Min", DoubleValue(0.0));
    radius3->SetAttribute("Max", DoubleValue(dSta)); // 10-meter radius for AP2
    mobility.SetPositionAllocator("ns3::RandomDiscPositionAllocator",
                                    "X", DoubleValue(-dAP), // AP2 at center
                                    "Y", DoubleValue(0.0),  // AP2 at center
                                    "Rho",StringValue("ns3::UniformRandomVariable[Min=0.0|Max=10.0]"));
...
{
    // Configure mobility for STAs of AP2 (randomly distributed around AP2 within 10 meters)
    Ptr<UniformRandomVariable> radius4 = CreateObject<UniformRandomVariable>();
    radius4->SetAttribute("Min", DoubleValue(0.0));
    radius4->SetAttribute("Max", DoubleValue(dSta)); // 10-meter radius for AP2
    mobility.SetPositionAllocator("ns3::RandomDiscPositionAllocator",
                                    "X", DoubleValue(0), // AP2 at center
                                    "Y", DoubleValue(dAP),  // AP2 at center
                                    "Rho",StringValue("ns3::UniformRandomVariable[Min=0.0|Max=10.0]"));
    mobility.SetMobilityModel("ns3::ConstantPositionMobilityModel");
    mobility.Install(wifiStaNodesD);
        ...
        if (nLinksB == 3)
        {
        std::cout << "Da5al el 3 for internet" << std::endl;
        ...
        std::cout << "traffic_load_BBS1 " << traffic_load_BBS1 << std::endl;
        ...
        if (nLinksB == 2 || nLinksB == 3)
        {   
            std::cout << "Da5al el 2 for APP" << std::endl;
            {
        ...
        if (nLinksB == 3)
        {  
            std::cout << "Da5al el 3 for APP" << std::endl;
        // BSS6
        {
