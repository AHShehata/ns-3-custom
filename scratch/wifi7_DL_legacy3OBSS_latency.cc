 ...
 */
 ...
 {   
     ...
     Ipv4Address target_sentadd("192.168.1.8");    // 2links
     //Ipv4Address target_sentadd("192.168.1.5");   // 1 link
     Ipv4Address destAddressIp = InetSocketAddress::ConvertFrom(destAddress).GetIpv4();
     if (srcAddress_corr == target_sentadd)
     {
 ...
 double ObssLoads[] = {0.7}; // Array with specific values
 ...
 for (std::size_t run = 0; run < Number_runs; ++run) 
 {
     ...
     {  
         ...
         {
         const auto maxRate =  EhtPhy::GetDataRate(mcs, channelWidth_24, gi, 1) / nStationsT;
